<?php
  ...
  class DB
  {
    ...
    function DB($base, $server, $user, $pass)
    {
      ...
      $this->conn = mysqli_connect($server, $user, $pass) or die('Server connexion not possible.');
      mysqli_select_db($this->conn,$base)               or die('Database connexion not possible.');
    ...
    {
      $this->nbQueries++;
      $res = mysqli_query($this->conn,$query) or $this->debugAndDie($query);
